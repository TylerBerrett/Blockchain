...
def mine():
    ...
    try:
        proof = data['proof']
        block_id = data['id']
        if block_id == "Tyler Berrett":
            message = "Failed to prove"
            block_string = json.dumps(blockchain.last_block, sort_keys=True)
            check = blockchain.valid_proof(block_string, proof, 6)
            if check:
                previous_hash = blockchain.hash(blockchain.last_block)
                blockchain.new_block(proof, previous_hash)
                message = "New Block Forged"
                blockchain.new_transaction("0", block_id, 1)
            response = {
                'message': message
            }
        ...
        response = {'Error': "Missing id or proof value"}
        return jsonify(response), 400
